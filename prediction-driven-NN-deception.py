...
if __name__ == '__main__':
    ...
    data_path = sys.path[0] + r"\data" + "\\" + strategy + "_deception" + "\\"
    ...
    # window = tk.Tk()
    # window.withdraw()
    # result = messagebox.askyesno("Continue execution?",
    #                              "Network parameters for all phases have been printed. Would you like to continue execution?")
    # if result:
    ...
    while (torch.any(opponents_s3 > end_symbol)):  # Sign of the end of the adversarial process
        if(Adv_phase < len(Adv_time) and start >= Adv_time[Adv_phase]):  # The opponent strategy changes and the adversarial process enters a new phase
            ...
            task = Env.task_symbol(P, Time, win_len)  # 实例化推演窗口任务序列
            ...
            # if deception and Adv_phase > 0:
            #         opponents_s3 = opponents_s3 / parameter.DLi[Adv_phase-1]
            ...
            if(opponents_number_change > 0):
                ...
                # change_index = [i for i in parameter.opponents_index[Adv_phase-1] if i not in opponents_index]
            ...
            retrain = True if strategy == "prediction-driven-NN" else False
            model_copies = []
            model = Model_fine_tuning.old_transfer_new(retrain, P, P_last_phase, Adv_phase)  # 预训练模型
            ae1, ae2 = Model_fine_tuning.old_transfer_new_ae(retrain, P, P_last_phase, Adv_phase)
            opponent_model = [model]
            if deception:
                opponent_model = [ae1, model, ae2]

            target_model = Model_fine_tuning.load_model(P, order, Adv_phase)
            target_ae1, target_ae2 = Model_fine_tuning.load_ae(P, Adv_phase)
        ...
        if(strategy == "Quantity-based"):
            ...
            u_opt, score = GWO.DEGWO(P, Searchwolf_num, Max_iter, KO_action, opponent_observed, Time, end_symbol,
                                     torch.Tensor(allies_s1_current_round), task, opponent_model, win_len, loss_type="Minimize Maximum")
        ...
        while (res != 0):
            ...
            res -= 1
        # u_opt = integer_array / Agent_tobe_departed_number
        ...
        if (retrain and ((opponent_his_data[-1, 0] - train_time) >= 0.2)):
            ...
            args_list = [(train_loader, model_copies[i], 2000, trainable_layers[i]) for i in range(parallel_fine_tune_num)]
            ...
            # print(max_fidelity_index)
            # print(loss)
            # print(model)
            # Model_fine_tuning.print_parameters_in_model(opponent_model[0])

            if not deception:
                torch.save(opponent_model[0].state_dict(),
                           model_path + str(Adv_phase-1) + "_deception" + "_retrain_number=" + str(retrain_number) + ".pkl")
            else:
                torch.save(opponent_model[0].state_dict(),
                           sys.path[0] + r"\trained_NN\ae1_Adv_phase=" + str(Adv_phase-1) + "_deception" + ".pkl")
                torch.save(opponent_model[1].state_dict(),
                           model_path + str(Adv_phase-1) + "_retrain_number=" + str(retrain_number) + "_deception" + ".pkl")
                torch.save(opponent_model[2].state_dict(),
                           sys.path[0] + r"\trained_NN\ae2_Adv_phase=" + str(Adv_phase-1) + "_deception" + ".pkl")

            np.save(data_path + "loss_Adv_phase=" + str(Adv_phase-1) + "_deception" + "_retrain_number=" + str(retrain_number)
            ...
            # if (opponent_his_data[-1, 0] > (opponent_his_data[0, 0] + 1.0)):
            #     retrain = False
            #     model = Model_fine_tuning.load_model(P, order, Adv_phase)
            #     ae1, ae2 = Model_fine_tuning.load_ae(P, Adv_phase)
            #     opponent_model = [model]
            #     if deception:
            #         opponent_model = [ae1, model, ae2]
