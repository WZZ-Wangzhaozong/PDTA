...
namespace Action_alg
{
    ...
    public class MyDataType
    {
        ...
        public Expression<Func<float, float, float, float, float, float, float>> Lambda { get; set; }  //能力分配实例化
        public Func<float, float, float, float, float, float, float> CompiledLambda { get; set; }

        public MyDataType(int intValue, float floatValue1, float floatValue2, Expression<Func<float, float, float, float, float, float, float>> lambda)
        {
            Loc = intValue;
    ...
    public class Program
    {
        static List<MyDataType> Alg(int P, float[] Time)
        {
            ...
            for (int i = 0; i < P; i++)
            {
                var func = Expression.Lambda<Func<float, float, float, float, float, float, float>>(U[i], U[0], U[1], U[2], U[3], U[4], U[5]);
            ...
            while (true)
            {
                ...
                if (Action_Lis[0].Ret_time > (3.5f + 30.157475345595586f / 62.08641773198754f)) break;
                ...
                for (int i = 0; i < P; i++)
                {
                    ...
                    var newFuncExpr = Expression.Lambda<Func<float, float, float, float, float, float, float>>(body, U[0], U[1], U[2], U[3], U[4], U[5]);
        ...
        static List<float> Num(List<MyDataType> Merged_Action_Lis, float[] pos, float N)
        {
            ...
            {
                Func<float, float, float, float, float, float, float> compiledLambda = item.Lambda.Compile();
                Merged_Action_Lis_return.Add(new MyDataType_return(item.Loc, item.Ret_time, item.Rec_time,
                                                                                                               compiledLambda(pos[0], pos[1], pos[2], pos[3], pos[4], pos[5]) * N));
            }
