        ...
        if (not retrain):
            ...
            if (opponents_number_change == 0):
                ...
                with torch.no_grad():
                    ...
                    # IE network transfer
                    indices = [parameter.opponents_index[Adv_phase - 1].index(x) for x in
                               parameter.opponents_index[Adv_phase]]
                    if (opponents_number_change < 0):
                        model.Son_swapnet.fc1.diagonal.data.copy_(model_ori.Son_swapnet.fc1.diagonal.data[indices])
                    else:
                        model.Son_swapnet.fc1.diagonal.data[:P_last_phase].copy_(
                            model_ori.Son_swapnet.fc1.diagonal.data)
                        model.Son_swapnet.fc1.diagonal.data[
                        P_last_phase:] = model_ori.Son_swapnet.fc1.diagonal.data.sum().item() / P_last_phase
            ...
            print("Phase " + str(Adv_phase - 1) + " opponent model has been transferred to the one in phase " + str(
                Adv_phase))
        return model
    ...
    def TD_layer_transfer(upper_tri_list_N, N, M, index_list):
        if (N > M):
            ...
            if len(upper_tri_list_N) != num_elements_N:
                raise ValueError(
                    "The given list length does not match the number of triangular elements on the N×N matrix")
        ...
        with torch.no_grad():
            if (len(target_model) == 1):  # Without deception of opponents
                ...
                for inputs, _ in test_loader:
                    ...
                    input_explicit_pow = torch.stack([input_explicit ** i for i in range(target_model[1].order)],
                                                     dim=-1)
                    ...
                    input_explicit_pow = torch.stack([input_explicit ** i for i in range(current_model[1].order)],
                                                     dim=-1)
...
if __name__ == '__main__':
    ...
    while (torch.any(anti_capability > end_symbol)):  # Sign of the end of the adversarial process
        if (Adv_phase < len(Adv_time) and adv_cur_time >= Adv_time[
            Adv_phase]):  # The opponent strategy changes and the adversarial process enters a new phase
            ...
            if (Adv_phase >= 1):
                ...
                opponent_exc_vel_data, opponent_rec_vel_data = Data_save.Calculate_velocity(P, opponent_his_data, KE,
                                                                                            KR, PNi, Bij, exp, beta)
                # Calculate the exchange and recovery amount of opponents.
                opponent_exc_data, opponent_rec_data = Data_save.Calculate_amount(opponent_exc_vel_data,
                                                                                  opponent_rec_vel_data)
                # Save data as npy.
                Data_save.Save_data_as_npy(data_path, [Adv_phase, KE, KR, KO], opponent_his_data, opponent_exc_data,
                                           opponent_rec_data,
                                           opponent_exc_vel_data, opponent_rec_vel_data, action_sequence, Ass_res_his)

            P_last_phase = parameter.P[Adv_phase - 1] if Adv_phase > 0 else parameter.P[
                0]  # Number of opponents in last phase
            ...
            Bij = parameter.Bij[np.ix_(parameter.opponents_index[Adv_phase],
                                       parameter.opponents_index[Adv_phase])]  # Bij considering solely the path length
            Bij = Initialize_each_phase.mask_matrix_by_labels(Bij,
                                                              Connect_graph)  # Updating Bij (Bij considering the path length and connectivity)
            args = (KE, KR, PNi, Bij, exp, beta, end_symbol)

            prediction_horizon = Initialize_each_phase.Prediction_duration_select(P, Time)
            task = Env.task_symbol(P, Time,
                                   prediction_horizon)  # Instantiate tasks within prediction horizon, including:
            ...
            if (opponents_number_change > 0):
                ...
                abandon_index = [i for i, x in enumerate(parameter.opponents_index[Adv_phase - 1]) if
                                 x not in opponents_index]
        ...
        KO_action = KO * Agent_tobe_ass[
            1, 0].item()  # Agent_tobe_ass[1, 0] represents the number of available agents / total number
        print("Actual Capability:" + str(KO_action))
        Agent_tobe_departed_number = int(
            round(Agent_tobe_ass[1, 0].item() * Agent_number))  # the number of available agents
        ...
        if (retrain and ((opponent_his_data[-1, 0] - train_time) >= 0.2)):
            ...
            for e in range(retrain_data.shape[0]):
                retrain_label[e, :] = Env.forward_equations(None, retrain_data[e, 1:], KE, KR, PNi, Bij, exp, beta,
                                                            None)
    ...
    opponent_his_data = Data_save.Sampling(opponent_his_data, given_intervals=0.05)
    # Calculate the exchange and recovery velocity of opponents.
    opponent_exc_vel_data, opponent_rec_vel_data = Data_save.Calculate_velocity(P, opponent_his_data, KE, KR, PNi, Bij,
                                                                                exp, beta)
    # Calculate the exchange and recovery amount of opponents.
