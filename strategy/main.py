...
class Initialize_each_phase:
    ...
    def loss_type_select(PNi, Bij, tol_PNi=0.1, tol_Bij=10000):
        ...
        '''
        PNi = 1 / PNi
        PNi = PNi / np.sum(PNi) * PNi.shape[0]  # Importance normalization
...
def main(strategy, deception=False, retrain=False):
    ...
    # 1. Creating a storage location for process data
    ...
    anti_capability = parameter.x0  # Initial anti-capabilities of opponents

    adv_cur_time = 0.0  # Current time of the adversarial process
    train_time = 0.0
    ...
    while (torch.any(anti_capability > end_symbol)):  # Sign of the end of the adversarial process
        if(Adv_phase < len(Adv_time) and adv_cur_time >= Adv_time[Adv_phase]):  # The opponent strategy changes and the adversarial process enters a new phase
            ...
            opponent_his_data = np.empty([0, P+1])  # anti-capabilities in this adversarial phase, including: [time, {opponents}]
            action_sequence = np.empty([0, P+1])  # assignment in this adversarial phase, including: [time, {opponents}]
            ...
            if strategy == "TNNLS":
                ...
                model = Model_fine_tuning.original_transfer_new(retrain, P, P_last_phase, Adv_phase)  # Pre-trained model/previous stage model
                encoder, decoder = Model_fine_tuning.original_transfer_new_coder(retrain, P, P_last_phase, Adv_phase)  # Pre-trained AE/previous stage AE
                ...
                target_encoder, target_decoder = Model_fine_tuning.load_encoder_decoder(P, Adv_phase)  # Well-trained AE
        ...
        # Calculating the assignment according to the strategy
        s1 = T.time()
        ...
        res = max(0, Agent_tobe_departed_number - np.sum(integer_array).item())  # Number of agents remaining to be assigned
        while (res != 0):
        ...
        if (retrain and ((opponent_his_data[-1, 0] - train_time) >= 0.2)):
            ...
            args_list = [(train_loader, model_copies[i], parameter.fine_tuning_epochs, trainable_layers[i]) for i in
...
KO = parameter.KO
Agent_number = parameter.Agent_number  # Agents' number
KE = parameter.KE  # Transfer ability factor
exp = parameter.exp  # Transfer index factor
KR = parameter.KR  # Recovery ability factor
beta = parameter.beta  # Recovery index factor
